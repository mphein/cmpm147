...
function p3_drawTile(i, j) {  
  ...
  // move tiles randomly
  let wobble = sin(random(0, millis()) * 0.005 + i + j);
  let shake = sin(random(0, millis()) * 0.005 + i + j);

  translate(shake, wobble);
...
function colorRow(i, j) {
  ...
  let n;
  if (map_factor == 0) {
    n = noise(i * 1, j * 1); // vertica lines more frequent changes
  } else {
      n = noise(i * .01, j * .01); // landscape
  }
